...
string openfilename() {
    //from https://stackoverflow.com/questions/18948783/c-simple-open-file-dialog-in-linux
    char filename[1024];
            ...
            if (cvui::button(frame, btn_start_w, btn_start_h + btn_height + btn_margin, btn_width, btn_height, "Image")) {
                if (mUseCamera) {
                    ...
                    if (mDestroyWindowFlag) {
                        while (cvGetWindowHandle("Show"))
                        {
                            cvDestroyWindow("Show");
                        }
                        
                        while (cvGetWindowHandle("Origin"))
                        {
    ...
    while (loop_flag) {
        ...
        if (mUseImage) {
            if (mImagePath != "") {
                ...
                string img_path = mImagePath.substr(0, mImagePath.length() - 1); // 文件对话框得到的字符串多了一个换行符
                
                captured_image = imread(img_path, CV_LOAD_IMAGE_COLOR);
