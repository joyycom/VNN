...
void drawGestureResult(cv::Mat used_img, VNN_GestureFrameDataArr gestureFrameData) {
	for (int gestureindex = 0; gestureindex < gestureFrameData.gestureNum; gestureindex++) {
		...
		cv::rectangle(used_img, cvPoint(left, top), cvPoint(right, bottom), CV_RGB(0, 255, 0), 2, 8, 0);
		...
		case VNN_GestureType_OneHandHeart:
			sprintf(gesture_type, "FingerHeart");
			break;
		...
		case VNN_GestureType_TwoHandHeart:
			sprintf(gesture_type, "HandHeart");
			break;
		...
		cv::putText(used_img, gesture_type, text_pos, cv::FONT_HERSHEY_COMPLEX_SMALL, 1.0, cv::Scalar(100, 80, 255), 1, 8, 0);

	}
	cv::imshow("Show", used_img);
	//cv::imwrite("gesture_thumbup.jpg", used_img);
