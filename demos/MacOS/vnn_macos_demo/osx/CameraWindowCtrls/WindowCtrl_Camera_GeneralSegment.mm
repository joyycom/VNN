...
#import "WindowCtrl_Camera_GeneralSegment.h"
...
@interface WindowCtrl_Camera_GeneralSegment ()
...
@implementation WindowCtrl_Camera_GeneralSegment
...
#   endif
    ...
    self.mtkView.mtltexture_bluredMask = [self.mtkView generateOffScreenTextureWithFormat:MTLPixelFormatA8Unorm width:_model_out_w height:_model_out_h];
...
#   if USE_GENERAL
    if (_handle > 0) {
        ...
        [self.mtkView.mtltexture_bluredMask replaceRegion:MTLRegionMake2D(0, 0, _model_out_w, _model_out_h)
                                              mipmapLevel:0
                                                withBytes:_outBuffer.get()->data
                                              bytesPerRow:_model_out_w];
        
        self.mtkView.mtltexture_offScreenImage = self.mtkView.mtltexture_background;
